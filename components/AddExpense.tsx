...
import { analyzeReceiptImage, parseInvoiceText, fileToGenerativePart } from '../services/geminiService';
import { fetchCarrierInvoices } from '../services/mofService';
import { Camera, Upload, Type, Save, X, Loader2, Sparkles, Plus, CloudDownload, QrCode } from 'lucide-react';

interface AddExpenseProps {
  onSave: (expenses: ExpenseDraft[]) => void;
  onCancel: () => void;
}

export const AddExpense: React.FC<AddExpenseProps> = ({ onSave, onCancel }) => {
  const [mode, setMode] = useState<'manual' | 'scan' | 'text' | 'carrier'>('manual');
  ...
  const updateDraft = (index: number, field: keyof ExpenseDraft, value: any) => {
        ...
        <>
          ...
          {mode === 'text' && (
            <div className="space-y-4">
                 ...
                 <button
                    onClick={() => {
                        const el = document.getElementById('invoice-text-input') as HTMLTextAreaElement;
